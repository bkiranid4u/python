...
class Date:
    ...
    _DAYS_NAME = ['MONDAY', 'TUESDAY', 'WEDNESDAY',
    ...
    def __init__(self, month, day, year):
        self._julianDay = 0
        assert self._isValidGregorian(
            month, day, year), "Invalid Gregorian date"
        tmp = 0
        if month < 3:
            tmp = -1
        # self._julianDay = day + floor((153*m + 2)/5) + 365*y + floor(y/4) - floor(y/100) + floor(y/400) - 32045
        self._julianDay = (1461 * (year + 4800 + tmp))//4 + (367 * (month - 2 -
                                                                    12 * (tmp)) // 12) - (3 * ((year + 4900 + tmp)//100))//4 + day - 32075
